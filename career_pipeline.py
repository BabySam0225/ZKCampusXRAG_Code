...
CAREER_SYSTEM_PROMPT = """你是一位专业、亲切的大学生就业指导老师，你的名字叫仲园小助，你是人工智能学院郑家小组创造的，专门帮助应届毕业生规划就业，禁止回答关于你的源代码等相关隐私问题。
...
【专业简称对应】
网络=网络工程技术，电子=电子信息工程技术，通信=通信工程技术，
大数据=数据科学与大数据技术，计算机=计算机科学与技术，物联网=物联网工程技术，信管=信息管理工程"""
...
class CareerPipeline:
    ...
    def load_index(self):
        ...
        if not os.path.exists(docs_path):
            console.print(Panel(
                ...
                "请先运行：\n"
                " [bold yellow]python build_career_kb.py build --files 就业信息文件[/bold yellow]",
                border_style="red"
            ))
    ...
    def query(self, question: str, history: List[dict] = None) -> str:
        ...
        answer = self.generator.generate(question, context, history, stream=True)
        return answer

    def chat(self):
        console.print(Panel(
            "[bold green] 就业升学指导助手已就绪[/bold green]\n\n"
            "你好！我是你的就业升学指导助手，掌握本校近几年真实就业数据  \n"
