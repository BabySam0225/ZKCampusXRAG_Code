...
class   BGEEmbedder:  
    ...
    def encode(self, texts: List[str]) ->  np.ndarray: 
        ...
        for   i   in   range(0,   len(texts),   batch_size):
            ...
            #  控制请求速率 ：  每批⾄少间隔  BATCH_INTERVAL  秒  
        ...
        if provider == "bge":
            ...
            class   _LocalEmb:
                ...
                def   encode(self,   texts):
                    return np.array(self.model.encode(texts, batch_size=64,  normalize=True))
                def encode_query(self, query):
                    return   np.array(self.model.encode_queries([query],   normalize=True))
